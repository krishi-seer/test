...
export default function AdvisoryPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const resized = await resizeIfNeeded(dataUrl);
  ...
  useEffect(() => {
    fetch("/api/weather").then(async (res) => {
      try {
        if (!res.ok) return;
        const { weather: w, aqi } = await res.json();
        const parts: string[] = [];
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      if (data.cropLabel && (data.confidence || 0) >= 0.3) {
        ...
        const currentLang = navigator.language?.startsWith('hi') ? 'hi' :
